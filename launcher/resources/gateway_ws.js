...
function GW_dispatchSessionEvent(event, payload, seq) {
  ...
  if (event === 'session.message' && _activeSessionCb) {
    ...
    // Extract text: prefer delta, then text field, then message.content
    const text = payload.delta || payload.text || (typeof msgContent === 'string' ? msgContent : '');
...
async function GW_sendMessage(sessionKey, messages, onChunk, onDone) {
  ...
  const userText = Array.isArray(messages)
    ...
    : messages;
  const sendResp = await GW_rpc('sessions.send', {
    sessionKey,
