...
function createAI(config, state, { log, utils }) {
  ...
  function proxyRequest(req, res, targetUrl, path, body, headers = {}) {
    ...
    const proxyHeaders = { ...req.headers, ...headers, host: u.host };
  ...
  function proxyToCloud(req, res, body) {
    try {
      ...
      const baseUrl = (config.cloud_api?.base_url || 'https://api.openai.com/v1').replace(/\/+$/, '');
      ...
      const reqBody = JSON.stringify({
        ...d,
        model: d.model || config.cloud_api?.model || 'gpt-4o-mini',
        messages: msgs
      });
