...
function createTTSRoutes(tts, config, state, { log, utils, audioDir, appRoot }) {
  ...
  // ===== Edge TTS 降级合成 =====
  async function generateWithEdgeTTS(text) {
    const voice = config.cloud_tts?.voice || 'zh-CN-XiaoxiaoNeural';
    const rate = config.cloud_tts?.rate || '+0%';
    log('INFO', 'Edge TTS: synthesizing with ' + voice);
    const mp3Buf = await edgeTTS.synthesize(text, { voice, rate });
    if (!mp3Buf || mp3Buf.length < 1000) {
  ...
  // ===== 本地 TTS + Edge TTS 降级 =====
  async function generateTTSWithFallback(text, customRef) {
    // 先尝试本地 GPT-SoVITS
    if (state.ttsReady && !state.ttsBreakerTripped) {
      try {
        return await tts.generateTTS(text, customRef);
      } catch (e) {
        log('WARN', 'Local TTS failed, falling back to Edge TTS: ' + e.message);
      }
    }
    // 降级到 Edge TTS
  ...
  function register(handlers) {
    ...
    handlers['POST /api/tts'] = async (req, res) => {
      ...
      try {
        ...
        const buf = await generateTTSWithFallback(text, Object.keys(customRef).length > 0 ? customRef : null);
        ...
        // Edge TTS returns MP3, local TTS returns WAV — both supported
        try { tts.saveAudioFile(buf); } catch {}
        sendAudio(res, buf);
      } catch (e) {
    ...
    handlers['POST /api/tts-generate'] = async (req, res) => {
      const body = await collectBody(req);
      try {
        ...
        const buf = await generateTTSWithFallback(text);
        if (!buf || buf.length <= 500) throw new Error(`invalid audio size=${buf ? buf.length : 'null'}`);
        const firstBytes = buf.slice(0, 10).toString('ascii').toLowerCase();
        if (firstBytes.includes('<!doctype') || firstBytes.includes('<html')) throw new Error('GPT-SoVITS error page');
