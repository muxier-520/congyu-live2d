...
// ==================== 模块引入 ====================
const { APP_ROOT, DATA_DIR, loadConfig, saveConfig } = require('./lib/config');
...
const state = {
  ...
  // 表情控制状态
  pendingExpression: null, pendingExpressionText: null, currentExpression: 'neutral'
};
...
// ==================== 服务初始化 ====================
const tts = createTTS(config, state, { appRoot: APP_ROOT, dataDir: DATA_DIR, audioDir: AUDIO_DIR, log, utils });
...
const adminRoutes = createAdminRoutes(tts, config, state, { log, logger: { recentLogs }, utils, appRoot: APP_ROOT, dataDir: DATA_DIR, audioDir: AUDIO_DIR });
...
const memoryService = createMemoryService(config, state, { log, utils });
const { createMemoryRoutes } = require('./routes/memory');
const memoryRoutes = createMemoryRoutes(memoryService, config, state, { log, utils });
memoryRoutes.register(handlers);

// Agent 工具调用系统
const { createAgent } = require('./services/agent');
const agent = createAgent(config, state, { log, utils });
const { createAgentRoutes } = require('./routes/agent');
const agentRoutes = createAgentRoutes(agent, config, state, { log, utils });
agentRoutes.register(handlers);
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.startsWith('/audio/') && req.method === 'GET') {
    const filename = decodeURIComponent(url.replace('/audio/', '').split('?')[0]);
